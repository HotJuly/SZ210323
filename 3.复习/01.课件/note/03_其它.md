...
- 3XX: 需要重定向, 浏览器直接跳转, 如302(重定向路径),304(缓存,重定向到内存中的资源)
- 4XX: 客户端请求错误, 如:  403(资源存在,但是权限不够,拒绝访问), 404(资源不存在,很有可能是请求路径错误)
- 5XX: 服务器端错误, 如: 500
...
​	ajax只是一种发送http请求的技术,他不是一种特殊类型的请求

- ajax(async javascript and xml)请求是一种特别的http请求
